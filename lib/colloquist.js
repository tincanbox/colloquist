...
module.exports = class colloquist {
  ...
  async init_lib(){
    ...
    [ 'puppeteer:puppeteer-core',
       ...
       * Always uses first prop as the vendor-name. (In above case, foo is vendor-name.)
       */
      let s = a.split(":");
      let v = s[0];
  ...
  async open(override){
    ...
    process.on('warning', (e) => {
      this.log_warn(FM.vr.stringify(e));
    });

    process.on('uncaughtRejection', (reason, p) => {
      this.log_error('CollqousitUnhandledRejection at: ' + p + ' , reason: ' + reason);
    });
  ...
  async execute(override){
    ...
    switch(this.command.action){
      ...
      case "tell":
        ...
        break;
      case "server":
        await this.server.start(this.command.option.name || "default", this.command.option.option || {});
        break;
      case "create":
      ...
      case "help":
        ...
        break;
      default:
        this.log_warn("Unknown command: " + this.command.action);
        break;
    }
  }
  ...
  async inhale(scenario){
    ...
    await this.logger.init_dictation(scenario);
  }
  ...
  async recite(draft, arg, override){
    ...
    try {
      ...
      this.log_error(e, (msg) => "Exception@colloquist.recite: " + msg);
      await this.mail.send_report('default/core.recite_exception', {
        subject: 'Scenario Error Report',
        data: { draft: d, arg: arg, result: r, error: e }
      });
    }finally{
      r.stack = scenario.reminiscence;
      let err = (scenario.reminiscence.reduce((m, s) => {m = m.concat(s.error); return m;}, []));
      if(err){
        r.error = err.map((e) => {
          return {
            message: e.message || e,
      ...
      if(r.stack){
        await this.mail.send_dictation_report(r.stack);
      }
    }
    return r;
  }
