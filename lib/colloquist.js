...
module.exports = class colloquist {
  ...
  async execute(override){
    ...
    switch(this.arg.command){
      case "run":
        await this.recite(this.arg.param.draft, this.arg.param.premise);
        break;
      case "server":
  ...
  /*
   */
  async recite(draft, param, override){
    if(!this.__is_opened) await this.open(override);

    this.log("PROCESS UUID: " + this.process_uuid);

    var d = draft || this.arg.param.draft || false;
    try {
      return await this.run(d, param);
    }catch(e) {
      e.message = "Exception@colloquist.recite: " + e.message;
      this.log_error(e);
      await this.mail.send_report('default/core.recite_exception', {
        subject: 'Scenario Error Report',
        data: { arg: this.arg, draft: d, result: this.result, error: e }
      });
      return this;
    }finally{
  ...
  async exit(){
    if(this.space){
      ...
      this.log("No Space is opened.");
    }

    this.log("<yellow>fin.</yellow>");

    if(this.result.error.length > 0){
      this.log("<red>Process finished with failures.</red>");
    }
