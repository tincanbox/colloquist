...
class colloquist {
  ...
  async open(override){
    ...
    // init other configs.
    var lacks = [];
    var confpath = this.config.path.config;
    for(any of ["path", "puppet", "spice", "mail"]){
      ...
      if(this.config.env){
        try{
          ...
          if(exists){
            this.config[any] = FM.ob.merge({}, this.config[any] || {}, exists, override);
          }
        }catch(e){
          // do nothing
        }
  ...
  async execute(){
    switch(this.arg.command){
      case "run":
        ...
        break;
      case "create":
        await this.copy_burden(process.cwd());
        break;
      case "version":
        this.logger.plain({
          level: 'info',
  ...
  async copy_burden(loc){
    await fsx.copy(
      [this.config.base, 'burden'].join(path.sep),
      [loc, 'burden'].join(path.sep),
      {
        overwrite: false
      }
    )
