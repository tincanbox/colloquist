...
class colloquist {
  constructor(option){
    ...
    global.FM = require([this.config.basedir, "lib","vendor","fmjs","src","fm"].join(path.sep));

    global.disc = (p) => {
      return require([this.config.basedir, 'lib', p].join(path.sep));
    }
  ...
  init(option){
    ...
    this.config = {
      basedir: path.resolve(__dirname + path.sep + '..')
    };
  ...
  async open(){
    ...
    for(any of ["path", "puppet", "spice", "mail"]){
      ...
      // check existence
      if(!this.config[any]){
        throw "Failed to fetch configuration " + any;
      }
    }

    require('dotenv').config(this.config.path.base);
