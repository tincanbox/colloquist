...
module.exports = class colloquist {
  ...
  init(option){
    ...
    /* init other configs.
     */
    this.require_config(base.concat(['config']), new RegExp("local\\.js"));
    if(this.config.stage){
      this.require_config(base.concat(['config', 'stage', this.config.stage]), this.config.list);
    }else{
      console.log("Notice: failed to retrieve config.stage value; can be supplied via burden/config/local.js");
    }
    this.require_config(base.concat(['config']), this.config.list);
    this.require_config(base.concat(['config']), new RegExp("(.*)\\.js"));
  ...
  async open(override){
    ...
    [ 'puppeteer:puppeteer-core',
      ...
      var s = a.split(":");
      this.vendor[s[0]] = require(s[1] || s[0]);
    });
  ...
  async recite(draft_name){
    try {
      var d = draft_name || this.arg.param.draft || false;
  ...
  async send_mail(host, param){
    ...
    if(!(host in this.config.mail)){
      throw new Error("mail host " + host + " is not assigned in config.");
    }
    ...
    try{
      ...
      // merged
      var m = FM.ob.merge(o, conf.default, param);
      // send
  ...
  async send_report(template, param){
    ...
    try{
      ...
      p.type = param.type || this.config.mail[h].default.type;
      p[p.type] = render(p.data || {});
      return await this.send_mail(h, p);
    }catch(e){
      this.log_error(e);
    }
  }
