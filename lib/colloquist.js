...
module.exports = class colloquist {
  ...
  constructor(option){
    ...
    this.command_behavior = {
      ...
      command: {
        ...
        run: [
          this.command_options.draft,
          this.command_options.param,
          this.command_options.headless,
          this.command_options.gui,
          this.command_options.slomo,
        ],
        tell: [
          this.command_options.name,
          this.command_options.param,
          this.command_options.headless,
          this.command_options.gui,
          this.command_options.slomo,
   ...
   */
  async init_core(){
    // Loads up pre-requisite vendors.
    let failed_vendor_list = [];
    [ 'puppeteer:puppeteer-core:puppeteer',
      'winston'
    ].map((a) => {
      /* Loads vendor engines with specific order.
       * foo:baz means, trying require(baz) first, and if it failed, then require(foo).
      ...
      for(let inc of s.reverse()){
        try{
          ...
          failed_vendor_list.push(v);
        }
      }
    });

    if(failed_vendor_list.length){
      for(let vendor_name of failed_vendor_list)
        if(!(vendor_name in this.vendor))
          throw "Failed to load " + vendor_name + ". Please confirm that `npm install` finished successfully.";
    }
    ...
    for(let k in cmd){
      switch(k){
        ...
        case "gui":
          ...
          break;
        case "param":
          cmd[k] = this.parse_argument_list(cmd[k]);
          break;
      }
    }
  ...
  parse_argument_list(list) {
    ...
    if(list_len){
      let result = list.reduce((stat, v, k) => {
        let vk = v.split("=");
        let is_last = ((k + 1) === list_len)
        switch (vk.length) {
          // single
          case 1:
            if (stat.previousIsKey) {
              stat.param.push(stat.previous + '=' + v);
              stat.previousIsKey = false;
            } else {
              if(is_last){
                stat.param.push(v + '=true');
              }else{
                stat.previousIsKey = true;
              }
            }
            break;
          // having `=`
          default:
            // Force boolify previous key only arg.
            if(stat.previousIsKey){
              stat.param.push(stat.previous + '=true');
              stat.previousIsKey = false;
            }
            // Here we go.
            stat.param.push(v);
      ...
      return FM.uri.unserialize(result.param.join('&')) || {};
    }else{
      return {};
    }
  }
