...
module.exports = class Story {
  ...
  async read(p, o){
    ...
    if(this.chapter.length == 0){
      this.abort("Chapter can not be empty!! You must register calling chapters via Story.compose() method. This registration should be done in constructor method. ");
    }

    if(this.reminiscene.length > 0){
      this.core.debug("Ack! I have to tell you important Stories first... About: " + this.reminiscene.join(", "));
      await this.core.scenario.scribe(this.reminiscene, param);
    }
  ...
  /* Registeres prerequisities chapters.
   */
  remini(story){
    story.map((e) => {
  ...
  async pull(url, opt){
    ...
    var res = await this.page.goto(url, opt);
    await this.prepare();
  ...
  monolog(n, v){
    this.core.logger.plain({
      level: 'debug',
      message: 'monolog: <yellow>' + n + '</yellow>' + ' -> ' + '<grey>' + FM.vr.stringify(v) + '</grey>'
    });
    this.remember(n, v);
  }
  ...
  _retrieve_premise(p){
    ...
    for(var tpl of this.premise_fullfillment){
      if(tpl.length == 2){
        o[tpl[0]] = tpl[1]
      }
    }
