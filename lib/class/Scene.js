...
module.exports = class Scene {
  constructor(scenario, draft, arg){
    ...
    this.space_autoload = FM.ob.thaw(draft, 'autoload', false);
    // Each Scenes use named session space.
    this.space_session = FM.ob.thaw(draft, 'session', 'default');
    // Toggles auto-close feature on Scenario run.
    this.space_keep = FM.ob.thaw(draft, 'keep', false);
    // Toggles instance will be opened as Tab|Window.
    this.space_isolated = FM.ob.thaw(draft, 'isolated', false);
  }
  ...
  async commence(){
    ...
    if(this.space_isolated){
      this.space = this.protocol.href;
      var cnt = await this.core.space.create(this.space);
      this.story.page = await this.core.space.insert(cnt);
    }else{
