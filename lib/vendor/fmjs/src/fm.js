(function(root, factory){
  ...
  _.vr.stringify = function(v){
    ...
    if(v instanceof Error){
      var e = {};
      e.message = v.message;
      e.stack = v.stack.split("\n").map(function(a){ return a.trim(); });
      v = e;
    }
    if(JSON){
      return JSON.stringify(v, rep());
    }else{
      if(v.toString){
        return v.toString();
      }
  ...
  _.ob.has = function(o, k){
    return (_.vr.is_o(o)) && o.hasOwnProperty(k);
  }
  ...
  _.async.poll = function(clb, count, interval){
    ...
    var p = _.async.promise(function(rs, rj){
      var main = function(){
        ...
        i++;
        if(i > c){
          return rj(new Error("FM.async.poll timedout"));
        }else{
          var stat = clb(rs, rj);
          (_.vr.is_pr(stat)) ? stat.finally(watch) : watch();
        }
      }
